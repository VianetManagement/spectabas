...
- **Language tracking** (v6.10.37): Every event payload carries `_lang` (`navigator.language` capped 16 chars, e.g. `"en-US"`) and `_plang` (`document.documentElement.lang` capped 16 chars, e.g. `"en"`). Server-side these populate the materialized columns `browser_language` + `page_language` on the events table (LowCardinality(String), bloom-filter indexed). The tracker's `sE()` builds `d.p = {_lang, _plang}` by default and merges caller-provided `x.p` keys on top, so custom-event payloads don't have to know about language fields and `_lang`/`_plang` stay on every event including `_click`, `_form_*`, etc. Dashboards via `Spectabas.Analytics.top_browser_languages/3`, `top_page_languages/3`, `language_country_crosstab/3`, `language_mismatches/3`, `language_summary/3`. Mismatch detection compares only the primary subtag (so `en-US` vs `en-GB` is a match; `en-US` vs `fr` is a mismatch). No backfill possible for pre-v6.10.37 events — they simply show empty.
- **SEO audit** (v6.10.47): per-page audit via a Playwright sidecar service (separate Render web service in `playwright-sidecar/`). `Spectabas.SEO.HeadlessClient.fetch/2` POSTs to `PLAYWRIGHT_URL`'s `/audit` endpoint → returns rendered HTML after `domcontentloaded` + up to 5s `networkidle`. `Spectabas.SEO.parse_and_score/3` parses (Floki) → extracts title/meta/H1/canonical/OG/JSON-LD/links/images → rule-based 0-100 score with critical (-20) / major (-10) / minor (-5) issues. `Spectabas.SEO.persist/1` writes to `page_audits` and prunes to 12 rows per (site, url). Worker: `Spectabas.Workers.SEOPageAudit`. Per-site weekly crawl budget via `sites.seo_crawl_budget` (default 500, range 100-5000); on-demand audits don't count. Floki is now a runtime dep (was test-only).
- **Form detail page** (v6.10.38): every row on the Forms list links to `/dashboard/sites/:id/forms/:form_id` (form_id URL-encoded). The detail page reads ten Analytics functions — `form_detail_kpis/4`, `form_timeseries/4`, `form_top_urls/4`, `form_breakdown/5`, `form_recent_events/5`, `form_submit_triggers/4`, `form_time_to_submit_distribution/4`, `form_validation_errors/4`, `form_field_times/4`, plus the existing `form_field_dropoff/4`. Breakdowns of enrichment fields (`device_type` / `ip_country` / `browser` / `utm_source` / `referrer_domain`) use the per-visitor subquery + anyIf pattern from CLAUDE.md's "Query Rules" because those fields are empty on custom events from the fast ingest path. `browser_language` is on every event so it groups directly. The Cohorts feature has two new Segment virtual fields, `form_submitted` and `form_abandoned`, taking the form_id as the value — both emit `visitor_id IN (...)` subqueries scoped via the existing `:site_id` opt on `Segment.to_sql/2`. The Form Detail page's "Create cohort of abandoners" button deep-links to the cohort builder with `?prefill_field=form_abandoned&prefill_value=<form_id>&prefill_name=...`. CohortsLive mount/3 reads those query params and prepopulates the new-cohort form.
- **Form tracking** (v6.10.35, cluster detection added v6.10.36, validation errors + field times added v6.10.38): Two kinds of form-like surfaces are tracked. **Kind 1 — `<form>` tags**: every `<form>` on the page fires `_form_view` on load, `_form_start` on first `focusin` inside, `_form_submit` on the browser's native `submit` event, `_form_abandon` on `visibilitychange-hidden`/`pagehide` if started but not submitted. **Kind 2 — Input clusters** (for React/Next.js apps that build forms out of standalone inputs + a submit button doing `fetch`): tracker scans for `<input>`/`<select>`/`<textarea>` not inside a `<form>`, groups them by their nearest identifiable ancestor (one with `id`, `data-testid`, `aria-label`, or a non-generated className ≥3 chars, walking up to 10 levels) that contains ≥2 trackable inputs. Each cluster is treated as a synthetic form. Submit is inferred from clicks on buttons whose text matches `/\\b(submit|send|sign[\\s_-]?up|sign[\\s_-]?in|log[\\s_-]?in|register|subscribe|continue|next|save|join|create[\\s_-]?account|get[\\s_-]?started|order|checkout|pay|buy)\\b/i` AND the click walks up to a cluster ancestor whose `started=true && submitted=false` — only fires once per cluster per pageview. Cluster cap: 20 per page. Heuristic submits carry `_submit_trigger: "button_text"` so the dashboard can flag them. All four event types carry `_form_kind` (`"form"` or `"cluster"`), `_form_id`, `_form_name`, `_form_action`. Form ID derives from `form.id` → `name:<form.name>` → `action:<url.pathname>` → `form:<index>`; cluster ID derives from container `id` → `dt:<data-testid>` → `al:<aria-label>` → `<tag>.<className>`. Powers the Forms dashboard page; queries via `Analytics.top_forms/3`, `form_analytics_summary/3` (returns `native_forms_tracked` / `clusters_tracked` / `heuristic_submits` splits), `form_field_dropoff/4` filtering on `event_type='custom' AND event_name IN ('_form_view','_form_start','_form_submit','_form_abandon')`.
...
- **Changelog**: v6.10.49 at `/admin/changelog`.
- **Insights stream**: `Spectabas.Insights` (context) + `insights` + `insight_dismissals` tables. Daily `Workers.InsightsGenerator` at 04:00 UTC turns `AnomalyCache` rows into Insight records (kind `anomaly_spike`/`anomaly_drop`) with per-`(site, kind, dedupe_key)` idempotency. `Workers.InsightExplainer` fires async per anomaly insight, calls Anthropic Haiku via the platform `HELP_AI_API_KEY` (not per-site AI config), and writes a 2-3-sentence explanation to `explanation`. UI: compact card on the Dashboard overview (top 5, click ✕ to dismiss per-user) + full feed at `/dashboard/sites/:id/insights-feed`. Dismissals are per-user via `insight_dismissals`. Goal-pace source is stubbed (returns `[]`) — add later by lifting goal_condition logic from `Analytics.goal_completions_system`.
- **Cohorts**: `Spectabas.Cohorts` (context) + `Spectabas.Cohorts.Cohort` (schema). Named, persisted segments — same filter grammar as `Spectabas.Analytics.Segment` so they reuse the existing `segment_filter` component + `segment_sql/1` translator. Visibility `"private"` (creator only) or `"site"` (everyone). UI at `/dashboard/sites/:id/cohorts` (list + builder), `/cohorts/:id` (detail metrics), `/cohorts/compare?a=&b=` (side-by-side). `Cohorts.metrics/3` aggregates overview_stats_fast + top_pages + top_sources + goal_completions for the detail/compare views. Cohort-scoped top_sources + cohort-scoped goal conversion are listed as follow-ups (the metric loader notes which fields are site-wide). Updated every push. Bumping a version means adding a new entry to the top of `ChangelogLive.entries/0` + updating this line. The Slack deploy notifier reads `ChangelogLive.current_version/0` at runtime (which returns the first entry's version), so the changelog is the single source of truth — no separate version constant to keep in sync.
