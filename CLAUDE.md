...
- **Scraper Detection** — weighted-signal scoring identifies likely scrapers: datacenter ASN (+35), spoofed mobile UA on datacenter IP (+20), IP rotation with same cookie (+20), 100+ unique pages (+20), 30+ unique pages (+10), systematic content crawl (+15), robotic request timing (+10), no referrer (+5), emulator resolution (+5). Score capped at 100; verdicts: :normal (<60), :suspicious (60-84), :certain (85+). Uses `uniqIf(url_path)` not `countIf` — refreshes/duration pings don't inflate scores. Per-site `scraper_content_prefixes` configurable in Settings. `Spectabas.Analytics.ScraperDetector` is a pure stateless module — no DB, no side effects. Dashboard shows summary cards + sortable table + click-to-detail modal with full UA, page paths, signals explained, and link to visitor profile. Visitor profile pages show real-time scraper score.
- **Scraper Webhooks** — per-site webhook (URL + Bearer secret) fires POST when a visitor crosses scraper threshold. Payload: visitor identifiers (IPs, ip_ranges, external_id, user_id), score, signals, activation_delay_hours (0 for score >= 95, 48 otherwise). When `datacenter_asn` signal fires, `ip_ranges` contains /64 CIDR prefixes for all IPv6 addresses (solves IPv6 rotation — scrapers cycling addresses within a /64 subnet get caught by prefix match). IPv4 addresses produce no ranges. Oban `ScraperWebhookScan` worker every 15 min. Tracks `scraper_webhook_sent_at` + `scraper_webhook_score` on visitors to send once, re-sends on score escalation. Manual Send/Deactivate buttons on Scrapers page. Deactivation POST to `/api/webhooks/spectabas/scraper/deactivate`. Visitor profile page shows webhook status banner (flagged/deactivated) and full webhook delivery history.
...
- **Changelog** — versioned changelog at `/admin/changelog`, updated on every push (current: v5.49.0)
...
- **VPN provider detection**: ipapi.is VPN MMDB databases (enumerated + interpolated) loaded via Geolix. `ip_vpn_provider` stores the provider name (e.g. "NordVPN", "ProtonVPN") in ClickHouse events. `ip_is_vpn` flag is set when either ASN blocklist OR MMDB matches. ScraperDetector suppresses `datacenter_asn` and `spoofed_mobile_ua` signals when visitor is on a known consumer VPN. Set `IPAPI_VPN_DIR` env var to the directory containing `enumerated-vpn.mmdb` and `interpolated-vpn.mmdb`. Health diag endpoint shows VPN database status.
