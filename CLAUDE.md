...
- **Changelog** — versioned changelog at `/admin/changelog`, updated on every push (current: v5.26.1)
...
- **Chart.js push timing gotcha**: `push_event` called during `mount` or inside a click handler that first renders new DOM fires *before* the client-side hook `mounted()` callback registers `handleEvent` listeners — the push is silently dropped and the chart renders an empty frame. Fix pattern: in `mount`, call `if connected?(socket), do: send(self(), :push_initial_charts)` and implement `handle_info(:push_initial_charts, ...)` that does the push. For drawers/modals rendered on click, the click handler should `send(self(), {:load_drawer, arg})` and a `handle_info` does the load + push. Also add `phx-update="ignore"` to chart container divs so LV diffs don't mangle the Chart.js-managed canvas. Affected so far: SiteLive, SearchKeywordsLive.
