...
- **Changelog** — versioned changelog at `/admin/changelog`, updated on every push (current: v5.23.0)
...
- **daily_rollup table**: AggregatingMergeTree with `countIfState` (pageviews), `uniqExactIfState(visitor_id)` (visitors), `uniqExactIfState(session_id)` (sessions), all filtered to `event_type='pageview' AND ip_is_bot=0`. Populated by `Spectabas.Workers.DailyRollup` (daily cron at 01:30 UTC rolls up yesterday; `%{"backfill" => true}` triggers a one-time historical backfill of all complete days). Idempotent via `ALTER TABLE daily_rollup DELETE WHERE date = X SETTINGS mutations_sync=2` before each insert. Query with `countIfMerge` / `uniqExactIfMerge` (note: -If combinator must stay in the Merge name).
