...
- **Changelog** — versioned changelog at `/admin/changelog`, updated on every push (current: v5.26.6)
...
- **ClickHouse alias shadowing = ILLEGAL_AGGREGATION**: Never alias an aggregate to the same name as the underlying column. `sum(clicks) AS clicks` then `sum(clicks)` elsewhere in the same SELECT makes the second `sum(clicks)` resolve to `sum(<the alias>)` = nested aggregation → `ILLEGAL_AGGREGATION (Code 184)`. Worse: with `FROM table FINAL`, ClickHouse sometimes silently returns 0 rows instead of the error, masking the bug. Fix: use distinct alias names like `total_clicks`, `daily_clicks`, etc. (Pattern used by query_stats and after-the-fact in query_daily_trends / drawer queries.)
