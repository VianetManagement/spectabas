...
- **ASN Management** — Self-managing datacenter ASN detection system. `ASNDiscovery` Oban worker runs weekly (Sunday 04:00 UTC), scans 30 days of traffic for unclassified hosting ASNs using behavioral scoring (engagement, bounce rate, org name keywords). Score >= 80 auto-added with ClickHouse backfill. Score 50-79 logged as inactive candidates for review. Admin page at `/admin/asn-management` with audit trail, manual add/approve/deactivate. Scraper detector uses `ASNBlocklist` ETS (~900 ASNs from priv/asn_lists/) for production scoring, static fallback list for tests. `asn_overrides` Postgres table tracks all changes.
...
- **Changelog** — versioned changelog at `/admin/changelog`, updated on every push (current: v5.68.0)
