...
- **Behavior**: Pages (device split, row evolution sparklines), Entry/Exit, Page Transitions, Site Search, Outbound Links, Downloads, Forms (auto-tracked, per-field abandonment), Events (ARRAY JOIN property breakdown), Performance (RUM/CWV)
...
- **Form tracking** (v6.10.35): Every `<form>` on the page auto-fires four custom events. `_form_view` on page load (form is in DOM), `_form_start` on first `focusin` inside the form (with `_first_field` = field that was focused, `_field_type`), `_form_submit` on `submit`, `_form_abandon` on `visibilitychange-hidden` / `pagehide` if started but not submitted (with `_last_field` = the most recently focused field). All four payloads carry `_form_id`, `_form_name`, `_form_action`. Form ID derives from `form.id`, then `name:<form.name>`, then `action:<url.pathname>`, then `form:<index>` — stable across reloads when at least one of those is set. Field name from `input.name` then `input.id` then `input.type`. Powers the Forms dashboard page; queries via `Analytics.top_forms/3`, `form_analytics_summary/3`, `form_field_dropoff/4` filtering on `event_type='custom' AND event_name IN ('_form_view','_form_start','_form_submit','_form_abandon')`.
...
- **Changelog**: v6.10.35 at `/admin/changelog`.
