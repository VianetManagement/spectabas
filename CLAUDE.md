...
- **Changelog**: v6.10.6 at `/admin/changelog`. Updated every push.
- **Click Elements snapshot**: `click_element_stats` Postgres table populated by `Spectabas.Workers.ClickElementSnapshot` (hourly cron, per-site jobs on `:maintenance` queue). LiveView reads from PG — sort/tag/search are PG queries, not ClickHouse. CH is hit at most once per site per hour with a 30-day window. Snapshot keeps top 1000 elements per site (`clicks >= 2`). Stale-by-up-to-an-hour is acceptable for a registry view. Don't add new code paths that re-query CH for the elements list — extend the worker instead.
