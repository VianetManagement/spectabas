...
- **Changelog**: v6.10.7 at `/admin/changelog`. Updated every push.
- **Snapshot pattern for slow dashboard tables**: Three Postgres tables back hot dashboard surfaces — `click_element_stats` (Click Elements page, 30d window, refreshed by `Spectabas.Workers.ClickElementSnapshot` at `:20`), `goal_stats` (Goals page completions + top sources, 7d window, refreshed by `Spectabas.Workers.GoalStatsSnapshot` at `:25`), `funnel_stats` (Funnels page entered/completed, 30d window, refreshed by `Spectabas.Workers.FunnelStatsSnapshot` at `:30`). All three workers fan out per-site jobs on the `:maintenance` queue. Creating a goal/funnel enqueues an immediate per-site snapshot. **Don't add new dashboard code paths that re-query CH for these lists** — extend the worker instead. Analytics has `_system` variants (`goal_completions_system/2`, `goal_source_attribution_system/3`, `funnel_summaries_system/3`) that skip the `authorize/2` check; use those from workers, not the user-facing functions.
