defmodule SpectabasWeb.Dashboard.TransitionsLive do
  ...
  alias Spectabas.{Accounts, Sites, Analytics, SEO}
  ...
  def mount(%{"site_id" => site_id} = params, _session, socket) do
    ...
    if !Accounts.can_access_site?(user, site) do
      ...
      socket =
        ...
        |> assign(:chart_key, 0)
        |> assign(:cache_key, 0)
        |> assign(:seo_audit, nil)
        |> assign(:seo_audit_queued, false)
  ...
  def handle_info(:load_data, socket) do
    ...
    # SEO audit lookup is a single fast PG read — no need for the
    # deferred-task pattern. Returns nil when the page has never
    # been audited, which the template handles by showing a
    # "Run audit" CTA.
    socket = load_seo_audit(socket)
  ...
  def handle_info(:poll_seo_audit, socket) do
    %{site: site, current_page: page} = socket.assigns
    url = SEO.build_audit_url(site, page)

    case SEO.latest(site.id, url) do
      %{captured_at: captured_at} = audit ->
        # If captured_at is within the last 90s, that's our new audit
        # finishing. Otherwise it's an older audit and we keep polling.
        if DateTime.diff(DateTime.utc_now(), captured_at, :second) < 90 do
          {:noreply,
           socket
           |> assign(:seo_audit, audit)
           |> assign(:seo_audit_queued, false)}
        else
          Process.send_after(self(), :poll_seo_audit, 5_000)
          {:noreply, socket}
        end
  ...
  defp load_seo_audit(socket) do
    %{site: site, current_page: page} = socket.assigns
    url = SEO.build_audit_url(site, page)
    audit = if url, do: SEO.latest(site.id, url), else: nil
    assign(socket, :seo_audit, audit)
  end
  ...
  def handle_event("run_seo_audit", _params, socket) do
    %{site: site, current_page: page, can_write: can_write} = socket.assigns
    url = SEO.build_audit_url(site, page)

    cond do
      not can_write ->
        {:noreply, put_flash(socket, :error, "Viewers can't trigger audits.")}

      is_nil(url) ->
        {:noreply, put_flash(socket, :error, "Couldn't build a URL for this page.")}

      true ->
        case SEO.enqueue_audit(site, url, trigger: "on_demand") do
          {:ok, _job} ->
            # Poll every 5s for up to 60s for the audit row to appear.
            Process.send_after(self(), :poll_seo_audit, 5_000)

            {:noreply,
             socket
             |> assign(:seo_audit_queued, true)
             |> put_flash(:info, "SEO audit queued for #{url}.")}

          {:error, reason} ->
            {:noreply, put_flash(socket, :error, "Couldn't queue audit: #{inspect(reason)}")}
        end
    end
  end
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow p-5">
            <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
              <div>
                <h3 class="font-semibold text-gray-900">SEO audit</h3>
                <p class="text-xs text-gray-500">
                  Real-browser audit of this page. Latest result shown below; click
                  <.link
                  ...
                  for the cross-page view.
                </p>
              </div>
              <button
                :if={@can_write and !@seo_audit_queued}
                phx-click="run_seo_audit"
                class="px-3 py-1.5 text-xs font-medium rounded-lg bg-indigo-600 text-white hover:bg-indigo-700"
              >
                {if @seo_audit, do: "Re-audit this page", else: "Run audit now"}
              </button>
              <span
                :if={@seo_audit_queued}
                class="inline-flex items-center gap-2 px-3 py-1.5 text-xs text-indigo-700 bg-indigo-50 rounded-lg"
              >
                ...
                Auditing… (auto-refreshes when complete)
              </span>
            </div>

            <div :if={is_nil(@seo_audit)} class="py-8 text-center text-sm text-gray-500">
              <p>No SEO audit for this page yet.</p>
              <p :if={@can_write} class="text-xs text-gray-400 mt-1">
                Click "Run audit now" above. On-demand audits don't count against your weekly crawl budget.
              </p>
            </div>
