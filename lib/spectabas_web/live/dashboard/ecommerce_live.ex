defmodule SpectabasWeb.Dashboard.EcommerceLive do
  ...
  def mount(%{"site_id" => site_id}, _session, socket) do
    ...
    if !Accounts.can_access_site?(user, site) do
      ...
      socket =
        ...
        |> assign(:loading, true)
        |> assign(:ecommerce, %{"total_orders" => 0, "total_revenue" => 0, "avg_order_value" => 0})
        |> assign(:top_products, [])
        |> assign(:orders, [])
        |> assign(:email_map, %{})
        |> assign(:timeseries, [])
  ...
  def handle_info(:load_data, socket) do
    {:noreply, socket |> load_ecommerce() |> assign(:loading, false)}
  end
        ...
        <%= if @loading do %>
          <div class="bg-white rounded-lg shadow p-12 text-center">
            <div class="inline-flex items-center gap-3 text-gray-600">
              ...
              <span class="text-sm">Loading ecommerce data...</span>
            </div>
          </div>
        <% else %>
          <%!-- Period Stats --%>
