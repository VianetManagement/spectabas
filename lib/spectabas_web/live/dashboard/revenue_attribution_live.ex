defmodule SpectabasWeb.Dashboard.RevenueAttributionLive do
  ...
  def mount(%{"site_id" => site_id}, _session, socket) do
    ...
    unless Accounts.can_access_site?(user, site) do
      ...
      socket =
        ...
        |> assign(:loading, true)
        |> assign(:rows, [])
        |> assign(:rows_unsorted, [])
        |> assign(:channels, [])
        |> assign(:total_revenue, 0)
        |> assign(:total_orders, 0)
        |> assign(:total_visitors, 0)
        |> assign(:ad_platforms, [])
        |> assign(:total_ad_revenue, 0)
        |> assign(:ad_campaigns, [])
        |> assign(:total_spend, 0)
        |> assign(:total_ad_clicks, 0)
  ...
  def handle_event("change_touch", %{"touch" => touch}, socket) do
    send(self(), :load_data)
    {:noreply, socket |> assign(:touch, touch) |> assign(:loading, true)}
  end
          ...
          <div :if={@has_ad_data} class="bg-white rounded-lg shadow p-5 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <dd class="text-lg font-bold text-gray-900">
                  {format_number(@total_ad_impressions)}
                </dd>
                <dd class="text-[10px] text-gray-400">reported by platform</dd>
              </div>
            </div>
