defmodule SpectabasWeb.Dashboard.OutboundLinksLive do
  ...
  def mount(%{"site_id" => site_id}, _session, socket) do
    ...
    unless Accounts.can_access_site?(user, site) do
      ...
      socket =
        socket
        |> assign(:page_title, "Outbound Links - #{site.name}")
        |> assign(:site, site)
        |> assign(:user, user)
  ...
  def render(assigns) do
    ...
    <.dashboard_layout
      flash={@flash}
      site={@site}
      page_title="Outbound Links"
      page_description="External links clicked by visitors, auto-tracked from outbound link clicks."
              ...
              <tbody class="divide-y divide-gray-200">
                <tr :if={@links == []}>
                  <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                    No outbound link clicks found. Outbound links are automatically tracked when visitors
                    click external links on your site.
                  </td>
                </tr>
                <tr :for={link <- @links} class="hover:bg-gray-50">
                  <td class="px-6 py-4 text-sm font-medium text-gray-900">{link["domain"]}</td>
                  <td class="px-6 py-4 text-sm text-gray-600 truncate max-w-xs" title={link["url"]}>
                    {link["url"]}
                  </td>
